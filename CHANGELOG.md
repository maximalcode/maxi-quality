...
- **`adopt.sh --agent --shared`** (#193) — 101 lines per repo instead of 984, with the scripts installed once at `~/.claude/agent-guard/` by `--install-shared`. Copying stays the default, so an outside adopter still gets a tree that works from one command. A missing shared body refuses rather than failing open. Declare your gate once in
