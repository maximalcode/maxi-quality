...
**Last updated:** 2026-08-07 · **Branches:** `develop` (default, where work
lands) → `main` (release) · **Tags:** `v1` (moving, follows `main`) · `v1.x.y`
(immutable — the full list is on
[Tags](https://github.com/maximalcode/maxi-quality/tags))
...
| **Two tags, on purpose** | `v1` is a *moving* pointer to the newest `v1.x` (the `actions/checkout@v4` convention) — that is how consumers pick up fixes without editing a workflow file, so moving it forward requires a force-push and that is expected, not an accident. It follows `main` automatically via `release-tag.yml`, which is why a merge to `main` is a release and contributions go to `develop` instead. The `v1.x.y` tags are immutable, for pinning something that can never shift; they are cut by hand, one per release worth naming. They are meant to carry a GitHub Release with the notes, and as of 2026-08-07 none of them does — the tags exist, the Releases were never cut. **Never attach a Release to `v1`** — it is force-pushed, so the notes would come to describe a different commit than the one they were written for. |
