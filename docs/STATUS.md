...
./scripts/scan.sh                        # expect exit 1, 32 semgrep findings / 19 rule ids
...
| **Semgrep is pinned twice** | `actions/layer2/action.yml` (what consumers get) and `ci.yml`'s `layer2-counts` (what validates the 32/19 assertion). They must match or CI is testing a tool nobody runs. `scripts/check-pins.sh --offline` guards it on every PR. |
...
files at the time of that scan (32 today — the ruleset gained fixtures and
